...
class LaserIndicator:
    ...
    def format_get_value_frame(self) -> bytes:
        ...
        address = 0x003B.to_bytes(length=2, byteorder="big")  # Starting address
        ...
        # Calculate CRC
        crc = self.calculate_crc(bytes(bytes_frame)).to_bytes(
    ...
    def format_set_zero_frame(self) -> bytes:
        ...
        bytes_frame = bytearray()
        bytes_frame.append(0x01)  # Address code
        bytes_frame.append(0x05)  # Function code

        register_address = 0x0002.to_bytes(
            length=2, byteorder="big"
        )  # Register address
        ...
        command = 0xFF00.to_bytes(length=2, byteorder="big")  # Command to set zero
            ...
            if self.serial.in_waiting > 0:
                ...
                if (
                    ...
                    if received_crc != calculated_crc:
                        response.pop(0)  # Remove the first byte and continue
                        print(
                            f"{self.name}: CRC mismatch. Expected {calculated_crc}, got {received_crc}."
                        )
                        continue

                    rearranged_bytes = bytes(
                        [response[5], response[6], response[3], response[4]]
                    )

                    # Extract the value from the data field
                    value = int.from_bytes(
                        rearranged_bytes, byteorder="big", signed=True
                    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="LaserIndicator Test Program")
    parser.add_argument(
        "--port",
        type=str,
        help="The serial port for the LaserIndicator, e.g., /dev/ttyUSB0. If not specified, it will try to find one automatically.",
    )
    ...
    if not port_to_use:
        ...
        for port in available_ports:
            try:
                sensor = LaserIndicator(f"{port} LaserIndicator", port)
                sensor.connect()
                try:
                    start_time = time.time()
                    while True:
                        value = sensor.get_value()
                        print(f"{sensor.name} value: {value}")
                        time.sleep(0.02)
        ...
        sensor = LaserIndicator("LaserIndicator", port_to_use)
        sensor.connect()
        try:
            # success = sensor.set_zero()
