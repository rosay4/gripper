...
class GripperGuide(BaseGuide):
    def __init__(self):
        ...
        self.laser_scan_interval = 2.0  # seconds
    ...
    def _scan_laser_ports(self):
        ...
        for p in ports:
            ...
            sensor = None
            try:
                sensor = LaserIndicator(name=f"Probe@{device}", port=device, baudrate=9600)
                sensor.connect()
    ...
    def _ensure_lasers(self):
        ...
        if len(ports) < 2:
            self.loggerUI.warn(f"laser scan: need 2 ports, found: {ports}")
            with self.feedback_lock:
