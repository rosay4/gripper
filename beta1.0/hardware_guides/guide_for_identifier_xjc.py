...
from keyboard_util2 import KeyboardCommander, input_with_flush
...
from visualize import draw,draw_wrench,draw_wrench_comp
import json
...
class Guide:
    ...
    def _get_launch_config(self):
        ...
        cfg.set("robot_model","")
        dict_planner = {
            "urdf": "/home/hillbot/RoboTest/beta1.0/model/hillbot_beta1_1.urdf",
    ...
    def select_arm(self):
        ...
        while True:
            ...
            if key in ["1", "2"]:
                ...
                break
        if self.selected_arm == "left_arm":
            self.urdf_file = "/home/hillbot/RoboTest/beta1.0/model/beta1_1_left_arm.urdf"
        else:
            self.urdf_file = "/home/hillbot/RoboTest/beta1.0/model/beta1_1_right_arm.urdf"
    ...
    def static_admittance_test(self):
        ...
        # === 切换导纳控制模式 ===
        input("right spot confirm?")
        self.robot.send_command(self.selected_arm,{
            "command": "set_right_spot"
        })
        print("切换至导纳控制模式中...")
        self.robot.send_command(self.selected_arm, {
            "command": "set_control_mode",
...
if __name__ == "__main__":
    ...
    config = {
        ...
        "root": {"level": "warn", "appenders": ["file"]},
        "loggers": {},
    }
    hblog.start(config)
    try:
