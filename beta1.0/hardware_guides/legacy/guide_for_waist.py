...
class guide:
    ...
    def _GetFeedback(self):
        while self.running:
            ...
            if feedInfo is not None:
                self.feedbackData.waist_pos = feedInfo[self.name]["position"]
                ...
                if self.record_flag.is_set():
                    self.highfreq_log.append({
                        ...
                        "highfreq_pos":self.feedbackData.waist_pos,
                    })
            time.sleep(0.001)
    ...
    def _run_point(self,pos):
        print("Moving...\n")
        while np.abs(self.feedbackData.waist_pos-np.array(pos)) > 1e-3:
    ...
    def cubic_move(self):
        start_point = float(self.feedbackData.waist_pos[0])
    ...
    def run(self):
        ...
        # 主菜单
        main_menu = {
            "1": ("验证腰部移动方向正确", self.task_verify),
            "2": ("20Hz腰部轨迹跟踪测试", self.task_tracking_error),
        }
        ...
        while self.running:
            os.system('clear')
            # --- 状态刷新 ---
            print(f"{GREEN}=== 腰部实时状态 ({self.name}) ==={RESET}")
            print(f"{GREEN}关节位置: {self.feedbackData.waist_pos}{RESET}")
