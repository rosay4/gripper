...
class guide:
    def __init__(self):
        ...
        # 状态管理
        self.running = True
        self.selected_force_sensor = "" ## "left_force_sensor" "right_force_sensor"
    ...
    def _GetFeedback(self):
        while not self._stop_event.is_set():
            ...
            if feedInfo is not None:
                sensor_data = feedInfo.get(self.selected_force_sensor)
                if sensor_data and 'force' in sensor_data:
                    self.feedbackData.Wrench = feedInfo[self.selected_force_sensor]['force']+feedInfo[self.selected_force_sensor]['torque']
                else:
                    # do nothing
                    time.sleep(0.01)
            time.sleep(0.005)
    ...
    def select_force_sensor(self):
        os.system('clear')
        print(f"{CYAN}=== 选择左/右侧六维力传感器 ==={RESET}")
        options = ["left_force_sensor","right_force_sensor"]
        ...
        while True:
            key = get_single_key()
            if key in ["1", "2"]:
                self.selected_force_sensor = ["left_force_sensor", "right_force_sensor"][int(key)-1]
                break
        config_path = str(BASE_DIR) + f"/configs/{self.selected_force_sensor}.yaml"
        self.config = ConfigManager(config_path=config_path)
        self.launch_config_dict = self._get_launch_config()
        print(f"✅ 已选择: {self.selected_force_sensor}")
        self._start()
    ...
    def task_verify(self):
        os.system('clear')
        key_path_bus = self.selected_force_sensor + ".usb_bus"
        key_path_ports = self.selected_force_sensor + ".usb_ports"
        task_steps = {
            "1": {
                "description": "修改bus配置",
                "callback": lambda: self.edit_config(key_path=key_path_bus),
            },
            "2": {
                "description": "修改ports配置",
                "callback": lambda: self.edit_config(key_path=key_path_ports),
            }
        }
        # 使用菜单栈
    ...
    def run(self):
        ...
        while self.running:
            os.system('clear')
            # --- 状态刷新 ---
            print(f"{GREEN}=== 传感器实时状态 ({self.selected_force_sensor}) ==={RESET}")
...
if __name__ == "__main__":
    try:
        force_sensor_guide = guide()
        force_sensor_guide.run()
    finally:
        print("shutdown...")
        force_sensor_guide.robot.shutdown()